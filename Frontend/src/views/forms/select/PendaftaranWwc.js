...
const DaftarWWC = () => {
    ...
    const readPendaftar = () => 
    axios.get(
      `${url}/api/pendaftaran-wawancaras?populate[peserta][populate]=pegawai&populate[pengujis][populate]=pegawai&populate=penilaians`)
    ...
    return (
        ...
        <CRow>
          ...
          <CCard className="mb-4">
            <CCardHeader>Jadwal Wawancara</CCardHeader>
            <CCardBody>
                  ...
                  {pendaftar.map((item,index) => (
                    <tr v-for="item in tableItems">
                     <td className="text-center">
                      ...
                      <td>
                      <div>{item.attributes.jenjang_jabatan_wwc}</div>
                      </td>
                      <td>
                      <div>{item.attributes.proyeksi_jabatan_wwc}</div>
                      </td>
                      <td className="text-center">
                      <div>{item.attributes.tanggal_wwc}</div>
                      </td>
                      <td align="center">
